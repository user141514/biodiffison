# filename: bioisosteric_v2_4_fragment_accfg.py
"""
方案A实现：片段替换扩散模型 - ACCFG版 + Kimi残差注意力

核心思想：
- 骨架（scaffold/Core）保持不变，不参与扩散
- 只对替换片段（官能团）进行扩散生成
- 使用ACCFG识别官能团边界，确定可断键位置（可选，默认关闭）
- Context = 骨架编码，更明确的条件信号

ACCFG分解逻辑（借鉴v2_3，可选）：
- 官能团识别：使用ACCFG/SMARTS识别分子中的官能团
- 可断键检测：官能团原子与Core原子之间的键
- 骨架 = Core（非官能团部分，通常是主环系统）
- 片段 = 官能团（等排体替换目标）
- 开关：CFG.use_accfg_decomposition（默认False，因为ACCFG很慢）

价键约束软惩罚（任务A+B）：
- 键类型权重：SINGLE=1, DOUBLE=2, TRIPLE=3, AROMATIC=1
- 原子类型-键类型联合约束：O/F/Cl/Br/I不应承受DOUBLE键
- 基于预测键概率计算期望价键负载

Kimi残差注意力（应用于CrossAttentionCondition）：
- 条件注入：output = h + Attention(h, cond)
- Pre-LN + Post-LN 双重归一化
- 多头注意力 + AdaGN调制
- 保留原始节点特征，避免信息丢失

模块层次：
- CrossAttentionCondition: 条件注入（Kimi残差注意力）
- EGNNLayer: 消息传递层（保持原有EGNN逻辑）
...
logger = logging.getLogger("BioIsosteric_V2_4_Fragment")
logger.setLevel(logging.INFO)
if not logger.handlers:
    formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
    fh = logging.FileHandler("bioisosteric_v2_4_fragment.log", encoding="utf-8")
...
class Config:
    ...
    test_mode: bool = True
    test_mode_limit: int = 5000  # 测试模式限制数据量
    ...
    # [新增V4] 自重建测试模式（诊断扩散模型是否正确）
    # ★ [P1修复] 课程学习配置
    # Phase 1: 自重建模式 - 学习完美重建原片段
    # Phase 2: 等排体生成模式 - 学习生成相似但不完全相同的片段
    self_reconstruction_mode: bool = True  # True=Phase 1(自重建), False=Phase 2(等排体)
    self_reconstruction_epochs: int = 10  # Phase 1训练轮数（避免过拟合）

    # ★ [P1新增] 课程学习第二阶段配置
    curriculum_phase: int = 1  # 1=自重建, 2=等排体生成（手动切换）
    isostere_fp_threshold: float = 0.35  # 等排体相似度阈值（比Stage2的0.4更宽松）
    isostere_epochs: int = 20  # Phase 2训练轮数

    # [新增V4] x0预测损失（片段重构损失）
    use_x0_prediction_loss: bool = True  # 是否启用x0预测损失
    x0_loss_weight: float = 0.05  # ★ [修复V3] 从1.0降到0.05，避免过拟合到数据集分布
    ...
    while True:
        ...
        if len(s_match) > 0:
            ...
            if o_count < 2:
                ...
                bonds = [(b.GetLength(), b) for b in atom.GetBonds()
        ...
        for f in files:
            ...
            for mol in suppl:
                ...
                if decomp is not None:
                    ...
                    decomp['scaffold_fp'] = torch.tensor(fp_array, dtype=torch.float32)

                    # 计算片段指纹（用于相似度匹配）
                    fragment_fp = AllChem.GetMorganFingerprintAsBitVect(
                        decomp['fragment'].mol, radius=2, nBits=2048
                    )
                    decomp['fragment_fp'] = fragment_fp
...
class BioIsostericFragmentModel(nn.Module):
    ...
    def train_stage3(self, dataset: FragmentDataset, epochs: int,
                     batch_size: int, pairs: List[Tuple[int, int]]):
        """
        Stage 3: 片段扩散训练（batch训练）

        目标：给定骨架编码，学习生成合适的片段
        [改进] 使用WeightedRandomSampler过采样稀有原子片段
        [新增V4] 自重建测试模式（课程学习第一阶段）
        [P1修复] 课程学习第二阶段：等排体生成模式
        """
        logger.info(f"Stage 3: Fragment diffusion training (batch), epochs={epochs}")

        # ========== [新增V4] 课程学习阶段选择 ==========
        if CFG.curriculum_phase == 1 or CFG.self_reconstruction_mode:
            # Phase 1: 自重建模式 - 学习完美重建原片段
            logger.info("[V4] Phase 1: Self-reconstruction mode (Curriculum Learning)")
        ...
        elif CFG.curriculum_phase == 2:
            # Phase 2: 等排体生成模式 - 使用更宽松阈值挖掘配对
            logger.info("[V4] Phase 2: Isosteric generation mode (Curriculum Learning)")
            # ★ [P1修复] 使用更宽松的阈值重新挖掘等排体配对
            pairs = mine_fragment_pairs(
                dataset,
                max_pairs=CFG.stage3_max_pairs,
                fp_threshold=CFG.isostere_fp_threshold  # 使用更宽松的阈值
            )
            if len(pairs) == 0:
                logger.warning("No isosteric pairs found, using self-reconstruction as fallback")
                pairs = [(i, i) for i in range(min(len(dataset), CFG.stage3_max_pairs))]
            logger.info(f"  Using {len(pairs)} isosteric pairs (scaffold_i -> fragment_j)")
            actual_epochs = min(epochs, CFG.isostere_epochs)
            logger.info(f"  Training for {actual_epochs} epochs")
        else:
            # 默认使用传入的配对
            if len(pairs) == 0:
                logger.warning("No pairs for Stage 3, using random sampling")
                pairs = [(i, i) for i in range(min(len(dataset), CFG.stage3_max_pairs))]
    ...
    def fit(self, dataset: FragmentDataset):
        ...
        # Stage 2
        pairs = self.train_stage2(dataset, CFG.stage2_epochs, CFG.stage2_batch_size)

        # Stage 3
        self.train_stage3(dataset, CFG.stage3_epochs, CFG.stage3_batch_size, pairs)
    ...
    def generate_fragment(self, scaffold_fp: torch.Tensor,
                          ...
                          scaffold_data: Optional[Data] = None) -> Dict:  # ★ 新增参数
        """
        给定骨架指纹，生成片段
    ...
    def reconstruct_molecule(self, scaffold_data: Data, fragment_result: Dict,
        ...
        try:
            ...
            if embed_result == 0:  # 嵌入成功
                mmff_result = AllChem.MMFFOptimizeMolecule(mol_h, maxIters=500)
...
def main():
    logger.info("=" * 60)
    logger.info("BioIsosteric V2.4 Fragment Diffusion Model")
    ...
    # 保存
    torch.save(model.state_dict(), "bioisosteric_fragment_model_v2_4.pth")
    logger.info("Model saved: bioisosteric_fragment_model_v2_4.pth")

    # 绘制损失曲线
    plot_loss_curves(model)

    # Demo推理
    demo_inference(model, dataset)
...
def plot_loss_curves(model):
    ...
    plt.tight_layout()
    plt.savefig('training_loss_curves_v2_4.png')
    logger.info("Loss curves saved: training_loss_curves_v2_4.png")


def demo_inference(model, dataset, num_samples=15):
    """Demo推理测试 - 显示ACCFG官能团信息，并保存结果到results文件夹"""
    from datetime import datetime

    # 生成带时间戳的结果文件名
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_filename = f"v2_4_results_{timestamp}.txt"
    ...
    with open(result_filepath, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write(f"Bioisosteric Fragment Diffusion Results - v2_4\n")
        ...
        for idx in range(min(num_samples, len(dataset))):
            ...
            result = model.generate_fragment(
                ...
                guidance_scale=3.0,  # ★ 提高：增强对原片段的忠实度
            )
