...
class Config:
    ...
    # Phase 1: 自重建模式 - 学习完美重建原片段（10 epochs）
    # Phase 2: 等排体生成模式 - 学习生成骨架相似+片段不同的替换（40 epochs）
    self_reconstruction_mode: bool = True
    self_reconstruction_epochs: int = 10  # Phase 1 训练轮数

    # ★ [V5新增] 等排体专用配置
    curriculum_phase: int = 1  # 1=Phase1自重建, 2=Phase2等排体（运行时切换）
    isostere_epochs: int = 40  # ★ Phase 2 训练更多轮（从20→40）
...
class BioIsostericFragmentModel(nn.Module):
    ...
    def fit(self, dataset: FragmentDataset):
        ...
        logger.info("Goal: Learn to perfectly reconstruct original fragments")
        logger.info("=" * 60)
        ...
        logger.info("Goal: Learn to generate scaffold-similar + fragment-different replacements")
        logger.info("=" * 60)
        ...
        logger.info("Phase 1: Self-reconstruction (10 epochs)")
        logger.info("Phase 2: Isostere generation (40 epochs)")
        logger.info("=" * 60)
